...
# Varijanta: LE – Laplacian Eigenmaps (generalizovani spektar L v = λ D v)

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
LE (Laplacian Eigenmaps): rešava L v = λ D v; embedding = sopstveni vektori
za k najmanjih ne-nula sopstvenih vrednosti (prvi je 0).
"""
...
def compute_le_embeddings(M, k=EMBED_DIM):
    ...
    idx = np.argsort(vals)[1 : 1 + k]
    emb = (D_inv_sqrt @ vecs[:, idx]).astype(float)
    for d in range(k):
        col = emb[:, d]
...
def main():
    ...
    emb = compute_le_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (LE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (LE + kvantna regresija, deterministički, strukturno):
    (8, 11, x, y, z, 34, 37)

    Score: 0.501884383636387
    """
...
L = D − A (ne-normalizovani Laplacian za čvorove 1..39), 
D = dijagonala stepena + 1e-12 (39×39).

Generalizovani spektar: 
rešava se L v = λ D v (np.linalg.eigh(L, D)) (39×39).

Embedding: 
k sopstvenih vektora za k sledećih najmanjih sopstvenih vrednosti (preskače se prvi, λ=0) (39×k).
